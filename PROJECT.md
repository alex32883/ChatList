...
То есть хранить в таблице на API-ключи, а имя переменной, где ключ лежит. Самые API-ключи будут в файле .env
...
Логика работы с моделями — в модуле model.py.
...
Поддержка разных типов моделей (OpenAI, Groq и т.д.).
Логи запросов.

7. Функция "AI-ассистент для улучшения промптов", которая:

Принимает исходный текст промпта от пользователя. Для этого сделаы отдельную кнопку.
Отправляет его в одну указанную модель (через существующий OpenRouter-клиент).
Возвращает:
улучшенную версию промпта;
2–3 варианта переформулировки;
при необходимости — адаптацию промпта под разные типы моделей (код, анализ, креатив).
Встраивает это в UI:
отдельная кнопка «Улучшить промпт» рядом с полем ввода;
...
Позволяет выбрать светлую/темную тему
Позволяет выбрать размер шрифта панелей
Все настройки сохраняются в таблицу settings в базе данных chatlist.db
